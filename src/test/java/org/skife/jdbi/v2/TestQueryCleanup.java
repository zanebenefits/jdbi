...
public class TestQueryCleanup
{
        ...
        throws Exception
    {
        ...
        final Query<Integer> q = handle.createQuery("SELECT COUNT(1) FROM something").map(IntegerColumnMapper.WRAPPER);
    ...
    {
        final Handle handle = dbi.open();
        final Query<Integer> q = handle.createQuery("SELECT id FROM something").map(IntegerColumnMapper.WRAPPER);
        final ResultIterator<Integer> it = q.iterator();
